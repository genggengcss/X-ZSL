...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred', default=os.path.join(DATA_DIR_PREFIX, EXP_NAME, 'save-gpm-att/epoch-680.pred'))
    ...
    for i, wnid in enumerate(test_wnids, 1):
        ...
        coefs = pred_coefs[all_label]
        # print 'coef shape:', len(coefs)
        weights = list()
        for l in range(len(coefs)):
            if coefs[l] > 1e-2:
                print WNID_WNAME[pred_wnids[l]]
                if pred_wnids[l] in train_wnids:
                    test = "- seen -" + WNID_WNAME[pred_wnids[l]] + "-" + str(float(coefs[l]))
                    weights.append(test)
                elif pred_wnids[l] in test_wnids:
                    test = "-unseen-" + WNID_WNAME[pred_wnids[l]] + "-" + str(float(coefs[l]))
                    weights.append(test)
                else:
                    test = "- none -" + WNID_WNAME[pred_wnids[l]] + "-" + str(float(coefs[l]))
                    weights.append(test)
        per_dict[wname].append(weights)
    ...
    # save to file
    per_class_acc = 'agcn_per_class_acc_weights1.txt'
