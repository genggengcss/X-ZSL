...
def embed_text_file(class_f, word_vectors, get_vector, save_file):
    with open(class_f) as fp:
    ...
    cnt_missed = 0
    missed_list = []
    for i, vertex in enumerate(class_list):
        class_name = vertex.lower()
        if i % 100 == 0:
            print('%d / %d : %s' % (i, len(class_list), class_name))
...
if __name__ == '__main__':
    ...
    save_file = os.path.join(DATA_DIR_PREFIX, EXP_NAME, 'glove_word2vec.pkl')

    # word embedding file
    word_vectors = get_glove_dict(os.path.join(DATA_DIR_PREFIX, 'materials'))
    get_vector = glove_google
    embed_text_file(class_file, word_vectors, get_vector, save_file)
