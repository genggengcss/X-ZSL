...
def sample_mask_sigmoid_inverse(idx, h, w):
    """Create mask."""
    mask = np.ones((h, w))
    matrix_zero = np.zeros((h, w))
    mask[idx, :] = matrix_zero[idx, :]
    return np.array(mask, dtype=np.bool)
...
# add self-connection to A, element:0 or 1
def adj_to_bias(adj, nhood=1):
    mt = np.eye(adj.shape[0])  # 3696  Diagonal matrix array
    for _ in range(nhood):
        mt = np.matmul(mt, (adj + np.eye(adj.shape[0])))
    mt[mt > 0.0] = 1.0
    # return -1e30 * (1.0 - mt)   # seen:0, unseen:infinitesimal
    return mt

def construct_feed_dict_agcn(features, support, labels, labels_mask, labels_adj_mask, val_mask, val_adj_mask, trainval_mask, trainval_adj_mask, biase_mask, placeholders):
    """Construct feed dictionary."""
    feed_dict = dict()
    feed_dict.update({placeholders['labels']: labels})
    feed_dict.update({placeholders['labels_mask']: labels_mask})
    feed_dict.update({placeholders['labels_adj_mask']: labels_adj_mask})
    feed_dict.update({placeholders['val_mask']: val_mask})
    feed_dict.update({placeholders['val_adj_mask']: val_adj_mask})
    feed_dict.update({placeholders['trainval_mask']: trainval_mask})
    feed_dict.update({placeholders['trainval_adj_mask']: trainval_adj_mask})
    feed_dict.update({placeholders['biase_mask']: biase_mask})
...
def construct_feed_dict(features, support, labels, labels_mask, labels_adj_mask, val_mask, val_adj_mask, trainval_mask, trainval_adj_mask, placeholders):
    """Construct feed dictionary."""
    feed_dict = dict()
    feed_dict.update({placeholders['labels']: labels})
    feed_dict.update({placeholders['labels_mask']: labels_mask})
