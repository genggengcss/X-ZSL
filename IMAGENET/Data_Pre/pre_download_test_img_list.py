...
IMAGE_SAVE_DIR = '/home/gyx/Data/Ori_DATA/ImageNet_2_hops_Images/'  # save directory
...
def download(vid_file):
    ...
    for i in range(len(vid_list)):
        ...
        save_dir = os.path.join(scratch_dir, wnid)  # 存储路径
...
def make_image_list(list_file, image_dir, name, offset):
    ...
    print(len(wnid_list))
    save_file = os.path.join(DATA_DIR_PREFIX, 'materials', 'test_img_list_%s.txt' % name)
...
def parse_arg():
    ...
    parser.add_argument('--save_dir', type=str, default=IMAGE_SAVE_DIR,
...
if __name__ == '__main__':
    if args.hop == '2':
        name = '2_hops'
        offset = 1000
        list_file = os.path.join(DATA_DIR_PREFIX, 'IMAGENET/materials', 'unseen_2012_2-hops.txt')
    elif args.hop == '3':
        name = '3_hops'
        list_file = os.path.join(DATA_DIR_PREFIX, 'IMAGENET/materials', 'unseen_2012_3-hops.txt')
    elif args.hop == 'all':
        name = 'all'
        list_file = os.path.join(DATA_DIR_PREFIX, 'IMAGENET/materials', 'unseen_2012_all.txt')
    else:
        raise NotImplementedError

    download(list_file)
    make_image_list(list_file, args.save_dir, name, offset)
