...
def add_atten_cos(input):
    # dot
    output = input
    output_T = output.t()
    logits = torch.matmul(output, output_T)
    print "logits shape: ", logits.shape
    coefs = F.softmax(logits, dim=0)
    output_atten = torch.matmul(coefs, input)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph',
                        default=os.path.join(DATA_DIR_PREFIX, EXP_NAME, 'imagenet-induced-graph.json'))
    ...
    parser.add_argument('--save_path', default=os.path.join(DATA_DIR_PREFIX, EXP_NAME, 'save-gpm-att'))
    ...
    fcfile = json.load(open(os.path.join(DATA_DIR_PREFIX, EXP_NAME, 'fc-weights.json'), 'r'))
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if epoch % args.evaluate_epoch == 0:
            ...
            train_loss = mask_l2_loss(output_vectors_atten, fc_vectors, tlist[:n_train]).item()
            if v_val > 0:
                val_loss = mask_l2_loss(output_vectors_atten, fc_vectors, tlist[n_train:]).item()
                loss = val_loss
            else:
                val_loss = 0
                loss = train_loss
